...
const diffDaysUTC = (date1: Date, date2: Date): number => {
...
const GanttChart: React.FC<{ tasks: GanttTask[] }> = ({ tasks }) => {
  ...
  const months = useMemo(() => {
    const monthMap = new Map<string, number>(); // Map<"YYYY-M", dayCount>
  ...
  const handleMouseDown = (e: React.MouseEvent, task: GanttTask, action: 'move' | 'resize-start' | 'resize-end') => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const daysHeader = Array.from({ length: totalDays }, (_, i) => {
    ...
    return (
      <div key={i} className="flex-shrink-0 w-8 text-center border-r border-gray-200 dark:border-gray-700 flex flex-col items-center py-0.5">
        <div className="text-xs text-gray-400">{['日', '一', '二', '三', '四', '五', '六'][day.getUTCDay()]}</div>
        <div className={`text-xs font-semibold w-5 h-5 flex items-center justify-center rounded-full ${isToday ? 'bg-blue-500 text-white' : ''}`}>
            {day.getUTCDate()}
        </div>
      </div>
    );
  ...
  const AddOrEditModal = ({ task, onClose }: { task?: GanttTask, onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-20 flex items-center justify-center" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-11/12 max-w-sm" onClick={e => e.stopPropagation()}>
                <h3 className="text-lg font-bold mb-6">{task ? '編輯安排' : '新增安排'}</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="mt-8 flex justify-between items-center">
                        <div>
                            {task && <button type="button" onClick={handleDelete} className="px-4 py-2 rounded-md bg-red-500 text-white text-sm font-semibold hover:bg-red-600">刪除</button>}
                        </div>
                        <div className="flex space-x-2">
                           <button type="button" onClick={onClose} className="px-4 py-2 rounded-md text-sm font-semibold">取消</button>
                           <button type="submit" className="px-4 py-2 rounded-md bg-blue-500 text-white text-sm font-semibold hover:bg-blue-600">儲存</button>
                        </div>
                    </div>
                </form>
            </div>
  ...
  return (
    <div className="p-4">
      {isAddModalOpen && <AddOrEditModal onClose={() => setIsAddModalOpen(false)} />}
      {editingTask && <AddOrEditModal task={editingTask} onClose={() => setEditingTask(null)} />}

      <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
        <div>
          <h3 className="text-lg font-bold">團隊甘特圖</h3>
        ...
        <div className="flex items-center space-x-2">
          <button onClick={handlePrev} className="p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700" aria-label="Previous 30 days">{'<'}</button>
          <button onClick={handleToday} className="p-2 px-3 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 text-sm">今天</button>
          <button onClick={handleNext} className="p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700" aria-label="Next 30 days">{'>'}</button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto" ref={scrollContainerRef}>
        <div style={{ width: 192 + totalDays * DAY_WIDTH }} className="relative">
          ...
          <div className="sticky top-0 bg-white dark:bg-black z-10">
              <div className="flex">
                  <div className="w-48 flex-shrink-0 p-2 font-semibold border-b border-r border-gray-200 dark:border-gray-700">任務</div>
                  <div className="flex-grow">
                      <div className="flex border-b border-gray-200 dark:border-gray-700">
                          {months.map((month, index) => (
                              <div key={index} style={{ width: month.width }} className="p-1 text-left pl-2 font-semibold text-sm border-r border-gray-200 dark:border-gray-700">
                                  {month.name}
                              </div>
                          ))}
                      </div>
                      <div className="flex">{daysHeader}</div>
                  </div>
              </div>
          </div>
          {/* Body */}
          <div className="relative">
            {tempTasks.map((task) => {
              ...
              return (
                <div key={task.id} className="flex items-center border-b border-gray-200 dark:border-gray-700 h-12 group">
                  <div className="w-48 flex-shrink-0 p-2 text-sm truncate border-r border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    <span className="truncate flex-grow" title={task.name}>{task.name}</span>
                    {isEditMode && 
                        <button onClick={() => setEditingTask(task)} className="ml-2 p-1 rounded-full text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700" aria-label={`Edit task ${task.name}`}><Icon name="edit" className="w-4 h-4"/></button>
                    }
                  </div>
                  <div className="flex-grow h-full relative">
                     <div
                      ...
                      >
                       {!isEditMode && <div className="absolute left-0 top-0 bottom-0 w-2 cursor-ew-resize" onMouseDown={(e) => handleMouseDown(e, task, 'resize-start')} />}
                       <span className="px-2 truncate pointer-events-none">{task.name}</span>
                       {!isEditMode && <div className="absolute right-0 top-0 bottom-0 w-2 cursor-ew-resize" onMouseDown={(e) => handleMouseDown(e, task, 'resize-end')} />}
                      </div>
                  </div>
                </div>
              );
